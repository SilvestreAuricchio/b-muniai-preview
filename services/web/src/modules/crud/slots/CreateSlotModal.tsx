...
interface Props {
  onClose: () => void
  onCreated: (slot: SlotRow) => void
}

export function CreateSlotModal({ onClose, onCreated }: Props) {
  const [hospitals,    setHospitals]    = useState<HospitalRow[]>([])
  const [hospitalUuid, setHospitalUuid] = useState('')
  const [date,         setDate]         = useState('')
  const [type,         setType]         = useState('')
  const [department,   setDepartment]   = useState('')
  const [medicinerCrm, setMedicinerCrm] = useState('')
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!hospitalUuid || !date || !type || !department) {
      setError('Hospital, Date, Type, and Department are required.')
      return
    }
    setSaving(true)
    setError(null)
    try {
      const slot = await api.post<SlotRow>('/slots', {
        hospital_uuid: hospitalUuid,
        date,
        type,
        department,
        mediciner_crm: medicinerCrm || undefined,
      })
      onCreated(slot)
    } catch (err: unknown) {
      const msg = (err as { data?: { error?: string } })?.data?.error ?? 'Failed to create slot.'
      setError(msg)
    } finally {
      setSaving(false)
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">New Slot</h2>
